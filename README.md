...
# Research Project: CIP, Food Welfare, and Smallholder Resilience in Rwanda
...
food welfare and resilience to agricultural and weather shocks.

## Data Sources
- **EICV7 (2023/24):** Household-level survey from World Bank Microdata Catalog
  - `CS_S01_S5_S7_Household.dta` — shock variables (s5eq1, s5eq2a, s5eq3a)
  - `CS_EICV7_poverty_file.dta` — food consumption, wealth quintiles, adult equivalents
...
## Analytical Option 1: Distress Coping Under Shock (CHOSEN FOR PROPOSAL)
**Research question:** Among households that experienced an agricultural or
weather shock, does living in a higher-LUC district increase the likelihood
of resorting to distress coping strategies?

**Sample:** Households reporting agricultural/weather shocks (s5eq1 == 1,
s5eq2a %in% c(1,2,3,4,5,6,8,9)), n ≈ 3,000

**Dependent variable:** Binary distress coping indicator from s5eq3a:
...
`distress_coping ~ luc_intensity + quintile_f + luc_intensity:quintile_f + ur_f + province_f`

**Rationale:** Directly tests the climate resilience argument — CIP strips
farmers of adaptive capacity (crop diversity, flexible plot allocation),
leaving them more exposed when climate shocks hit. Connects to seminar
themes of climate change, conflict, and structural vulnerability. Motivated
by Clay and Zimmerer (2020) and Matin et al. (2018).

**Known limitation:** District-level LUC did not reach significance in test
regression (p = 0.23). May reflect insufficient within-province variation in
LUC intensity, or heterogeneity in shock severity across households.
Household-level CIP participation data would strengthen identification but
is not available in EICV7.

---

## Analytical Option 2: Baseline Food Consumption (ALTERNATIVE)
**Research question:** Does living in a higher-LUC district reduce household
food consumption per adult equivalent?

**Sample:** Full nationally representative EICV7 sample, n ≈ 15,000

**Dependent variable:** Log food consumption per adult equivalent
(`log(food / ae)`) from EICV7 poverty file

**Model:** OLS regression
`log(food_ae) ~ luc_intensity + quintile_f + luc_intensity:quintile_f + ur_f + province_f`

**Rationale:** Extends Del Prete et al. (2019) with more recent data (EICV7
vs EICV3/4) and a more exogenous treatment variable (district-level intensity
vs household self-selection). Cleaner identification than Option 1.

**Test result:** luc_intensity significant at p = 0.017, negative direction
as expected. Interaction terms not significant but main effect is clean.

**Known limitation:** Does not directly test resilience or shock exposure.
Less theoretically aligned with climate change and conflict seminar context.
